# Tutorial 01: Hello World Agent - Structure Tests
...
class TestProjectStructure:
    ...
    def test_hello_agent_directory_exists(self):
        """Test that hello_agent directory exists."""
        assert os.path.isdir('hello_agent'), "hello_agent directory not found"

    def test_init_py_exists(self):
        """Test that __init__.py exists in hello_agent."""
        init_file = os.path.join('hello_agent', '__init__.py')
        assert os.path.isfile(init_file), "__init__.py not found in hello_agent"

    def test_agent_py_exists(self):
        """Test that agent.py exists in hello_agent."""
        agent_file = os.path.join('hello_agent', 'agent.py')
        assert os.path.isfile(agent_file), "agent.py not found in hello_agent"

    def test_env_example_exists(self):
        """Test that .env.example exists in hello_agent."""
        env_file = os.path.join('hello_agent', '.env.example')
        assert os.path.isfile(env_file), ".env.example not found in hello_agent"
    ...
    def test_env_example_content(self):
        ...
        assert "# Get your free API key" in content
...
class TestRootFiles:
    ...
    def test_readme_content(self):
        ...
        assert "Tutorial 01" in content
        assert "Hello World Agent" in content
